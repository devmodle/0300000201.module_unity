...
os.system(f"python ModuleCmdExecuter.py \"{oProjName}\" \"git add .\"")
oCommitMsg = f"\"{oCommitMsg}\"" if "WINDOWS" in platform.system().upper() else oCommitMsg

# 윈도우즈 플랫폼 일 경우
if "WINDOWS" in platform.system().upper():
	os.system(f"python ModuleCmdExecuter.py \"{oProjName}\" \"git commit -m \"{oCommitMsg}\"\"")
else:
	os.system(f"python ModuleCmdExecuter.py \"{oProjName}\" \"git commit -m \'{oCommitMsg}\'\"")

# 브랜치 이름이 유효 할 경우
if oBranchName:
	os.system(f"python ModuleCmdExecuter.py \"{oProjName}\" \"git push origin -u {oBranchName}\"")
else:
